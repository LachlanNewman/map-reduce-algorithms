...
public class WordPair implements Writable, WritableComparable<WordPair> {
    ...
    public int compareTo(WordPair other) {
        int returnVal = this.word.toString().compareTo(other.getWord().toString());
        if(returnVal == 0) {
            returnVal = this.neighbor.toString().compareTo(other.neighbor.toString());
            if (this.neighbor.toString().equals("*") && !other.neighbor.toString().equals("*")){
                returnVal = -1;
            }
            if (!this.neighbor.toString().equals("*") && other.neighbor.toString().equals("*")){
                returnVal = 1;
